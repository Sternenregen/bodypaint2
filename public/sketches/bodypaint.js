...
function setup() {
  ...
  //set my color
  myColor = [random(0, 170), random(0, 170), random(0, 170)]; //max 170, to avoid colors to close to white!
}
...
function draw() {
  ...
  fillInSpaces(prevX, prevY, ex, ey, myColor[0], myColor[1], myColor[2]);
...
function fillInSpaces(fx, fy, pfx, pfy, r, g, b) { //tries to fill in spaces between ellipses with a line 
  stroke(r, g, b);
  strokeWeight(w);
  noFill();
  line(pfx, pfy, fx, fy);
...
function newDrawing(data){
  ...
  fillInSpaces(data.pX, data.pY, data.x, data.y, data.color[0], data.color[1], data.color[2]);
